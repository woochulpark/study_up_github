...
$("input[name='pet_cate']").click(function(){
	...
	if(dogchk){
		...
		$('.ins_search_box .chk_dog_regi').css('display','block');	
	}else{
		$('.ins_search_box .chk_dog_regi').css('display','none');
		$('.chk_dog_size input[name=dog_regi]').prop('checked',false);//191023 추가
	}
})

function chk_section(kind){
	
	if(kind > 0){
		var pic_life = $('#ui-datepicker-div').css('display');
		if(pic_life == "block"){
			$('#petbirth').datepicker('hide');
			//$('body').focus();
			$('#petbirth').blur();
		}
	}
